...
import com.Benhanan14406.dragon.entities.CustomPathFinder;
...
public class CustomGroundPathNavigation extends GroundPathNavigation {
    public CustomGroundPathNavigation(@NotNull Mob entity, Level world) {
        super(entity, world);
    }
        ...
        do {
            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    ...
                    PathType below = this.nodeEvaluator.getPathType(this.mob, this.mob.getOnPos());
                    if (below == PathType.WATER || below == PathType.LAVA || below == PathType.OPEN) return false;
                    PathType in = this.nodeEvaluator.getPathType(this.mob, this.mob.getOnPos());
